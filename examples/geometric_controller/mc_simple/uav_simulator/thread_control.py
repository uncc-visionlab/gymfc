...
def thread_control():
    ...
    # while not rospy.is_shutdown() and rover.on:
        ...
        if (not rover.motor_on) or (rover.mode < 2):
            fM_message = Wrench(force=Vector3(x=0.0, y=0.0, z=0.0), torque=Vector3(x=0.0, y=0.0, z=0.0))
        else:
            fM_message = Wrench(force=Vector3(x=0.0, y=0.0, z=fM[0]), torque=Vector3(x=fM[1], y=fM[2], z=fM[3]))
