...
namespace gazebo {
    ...
    class UavControlPlugin : public ModelPlugin {
        typedef const boost::shared_ptr<const geometry_msgs::msgs::Wrench> WrenchPtr;

    public:
        void Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
            gzdbg << "Loading control plugin\n";
            // Get node handle.
            n = transport::NodePtr(new transport::Node());
            ...
            this->sub_fm = this->n->Subscribe<geometry_msgs::msgs::Wrench>(this->topic_name,
                 &UavControlPlugin::update_fm, this);
        }
        ...
        static void update_fm(WrenchPtr &msg) {
//            gzdbg << __FUNCTION__ << "Received force " << msg->force().z() << " and torque = (" << msg->torque.x()
//                  << ", " << msg->torque.y() << ", " << msg->torque.z() << ")" << std::endl;
            f[0] = msg->force().x();
            f[1] = msg->force().y();
            f[2] = msg->force().z();
        ...
        void ignition_to_eigen(const ignition::math::Vector3d input, fdcl::Vector3 &output) {
            output(0) = input[0];
            output(1) = input[1];
            output(2) = input[2];
        }
        ...
        void eigen_to_ignition(const fdcl::Vector3 input, ignition::math::Vector3d &output) {
            output[0] = input(0);
            output[1] = input(1);
            output[2] = input(2);
        }
...
//        ros::NodeHandle n;
        gazebo::transport::SubscriberPtr sub_fm;
        transport::NodePtr n;
