...
void fdcl::eigen_to_ignition(const Vector3 input, ignition::math::Vector3d &output) {
    output[0] = input(0);
    output[1] = input(1);
    output[2] = input(2);
}
...
void fdcl::ignition_to_eigen(const ignition::math::Vector3d input, Vector3 &output) {
    output(1) = input[0];
    output(2) = input[1];
    output(3) = input[2];
}
